// John Remy: this is the gallery's content source — the wall on which the work hangs.
// It mirrors the Sanity `project` schema field-for-field, so the day content moves to
// the CMS, the components don't change. Replace these starter entries with your real
// work (or wire `sanityFetch`); keep descriptions about the craft, never fake metrics.
...
export interface Project {
  ...
  description: string[];
  tags: string[];
  /** Public path or Sanity URL. Omit to render the typographic cover. */
  coverImage?: string;
  liveUrl?: string;
  sourceUrl?: string;
  featured?: boolean;
